...
//					console.log($scope.IndexImag);
		...
		$('#sdt_menu > li').bind('mouseenter', function() {
		   ...
		   $(".platformText li").eq(0).find(".spanTitle").addClass("on");
           $(".platformUl").slide({ titCell:".platformText li .spanTitle", mainCell:".platformImg",effect:"fold", autoPlay:true});
		});
		...
		$scope.getExPlan = function() {
			...
			$http.get(HOST_URL + "/plans?order=desc&type=" + 1 + "&limit=" + 1).success(function(responseData) {
				...
				if(responseData.resultCode == "0") {
					...
					$scope.plans.percent=parseInt((($scope.plans[0]['nowSum'] / $scope.plans[0]['amount']) * 100)>100 ? 100:($scope.plans[0]['nowSum'] / $scope.plans[0]['amount']*100));
				 
				$scope.newPlans = {
        "width" : $scope.plans.percent+"%",
...
//				radialIndicatorInstance['indicator1'].animate(a);
		...
		$scope.getNews = function() {
			$http.get(HOST_URL + "/news/list?limit=7&page=" + 1).success(function(responseData) {
				if(responseData.resultCode == "0") {
